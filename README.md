...
   После успешной сборки перенесите (или установите) скомпилированный файл ядра в нужное место и настройте WSL для его использования (например, отредактировав файл \`.wslconfig\`). Затем перезапустите WSL:
   \`\`\`powershell
   wsl --shutdown
   \`\`\`
