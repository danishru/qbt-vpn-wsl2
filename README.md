...
Обратите внимание, что если CIFS-папка содержит каталоги, представляющие смонтированные диски, они могут быть недоступны для Docker до тех пор, пока их не "пробудят" командой ls -ld в системе WSL. Для автоматического обновления состояния таких папок после запуска WSL создайте следующий скрипт:
