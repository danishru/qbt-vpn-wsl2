...
8. Создайте папку для нового ядра и скопируйте образ ядра в файловую систему Windows:
   ...
   sudo cp arch/x86/boot/bzImage /mnt/c/kernel/wsl_6.6_full
   ```
...
После успешной сборки перенесите (или установите) скомпилированный файл ядра в нужное место и настройте WSL для его использования, отредактировав файл \`.wslconfig\` в вашем профиле Windows. Например, если вы скопировали образ ядра по пути `/mnt/c/kernel/wsl_6.6_full` (этот путь указан как пример, вы можете использовать любой удобный), выполните следующие действия:

1. Откройте PowerShell от имени администратора и выполните команду для открытия файла `.wslconfig` в Notepad:
   ```powershell
   notepad $env:USERPROFILE\.wslconfig
   ```
2. В файле `.wslconfig` найдите раздел `[wsl2]` и дополните его, добавив строку с указанием нового ядра. Например, если ваш новый образ ядра находится по пути `C:\kernel\wsl_6.6_full`, обновлённый раздел должен выглядеть так:
   ```ini
   [wsl2]
   kernel=C:\\kernel\\wsl_6.6_full
   ```
> [!NOTE]  
