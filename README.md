...
### Шаг 5. Опционально: Сборка последнего ядра Linux с поддержкой WSL2 и Docker

Если вы хотите воспользоваться всеми новыми возможностями и улучшенной производительностью, предлагаем собрать собственное ядро Linux. Этот шаг особенно полезен для обеспечения оптимальной поддержки WSL2 и Docker.

> [!WARNING]  
> Сборка и установка нового ядра требует значительных ресурсов и опыта. Перед выполнением данных действий рекомендуется создать резервные копии важных данных. Если вы не уверены в своих действиях, воспользуйтесь стандартным ядром, поставляемым вашим дистрибутивом, или обратитесь за помощью к специалистам.

1. **Резервное копирование текущего дистрибутива Linux в WSL2:**  
   Выполните следующую команду в Windows CMD или PowerShell:
   ```powershell
   wsl --vhd --export Debian <Путь_к_файлу.vhdx>
   ```
   Замените `<Путь_к_файлу.vhdx>` — на путь, где вы хотите сохранить резервную копию в формате VHDX.
